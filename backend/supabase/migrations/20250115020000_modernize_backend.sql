-- Migration: Modernize Supabase Backend with 2024/2025 Best Practices
...
-- Index for recent tournament selections
CREATE INDEX IF NOT EXISTS idx_tournament_selections_recent 
...
SELECT 
  ...
  SUM(cnr.wins) as total_wins,
  SUM(cnr.losses) as total_losses,
  COUNT(ts.id) as tournament_selections,
  cau.created_at as joined_at,
  MAX(GREATEST(cnr.updated_at, ts.selected_at)) as last_activity
FROM cat_app_users cau
LEFT JOIN cat_name_ratings cnr ON cau.user_name = cnr.user_name
LEFT JOIN tournament_selections ts ON cau.user_name = ts.user_name
GROUP BY cau.user_name, cau.user_role, cau.created_at;

-- Create index on materialized view
CREATE INDEX IF NOT EXISTS idx_user_activity_stats_activity 
...
COMMENT ON MATERIALIZED VIEW user_activity_stats IS 'Pre-computed user activity statistics for analytics';
