 ...
 * Classes/Functions:
 *   - if (!isOpen) (line 180)
 *   - const calculateStats = (ratings, filterStatus = FILTER_OPTIONS.STATUS.ALL) => (line 206)
 *   - if (filterStatus === FILTER_OPTIONS.STATUS.ACTIVE) (line 208)
 *   - if (filterStatus === FILTER_OPTIONS.STATUS.HIDDEN) (line 211)
 *   - const processUserRatings = (data) => (line 249)
 *   - if (!data) (line 253)
 *   - if (!item.name_options) (line 258)
 *   - if (!ratingsByUser[userName]) (line 273)
 *   - onUserAction("export", user); (line 508)
 *   - onUserAction("delete", user); (line 516)
 *   - const AdminPanel = ( (line 527)
 *   - if (activeTimeFilter !== "all") (line 536)
 *   - switch (activeTimeFilter) (line 540)
 *   - switch (sortBy) (line 563)
 *   - setSortOrder((order) => (order === "asc" ? "desc" : "asc")) (line 635)
 *   - if (name.averageRating >= 1800) (line 745)
 *   - useEffect(() => (line 902)
 *   - const fetchHiddenNames = async () => (line 903)
 *   - if (hiddenError) (line 909)
 *   - setHiddenNames(newHiddenNames); (line 916)
 *   - fetchHiddenNames(); (line 922)
 *   - if (!rating) (line 931)
 *   - if (filterStatus === FILTER_OPTIONS.STATUS.ACTIVE && isHidden) (line 938)
 *   - if (filterStatus === FILTER_OPTIONS.STATUS.HIDDEN && !isHidden) (line 941)
 *   - if (!nameMap.has(rating.name)) (line 945)
 *   - switch (sortBy) (line 994)
 *   - const prepareChartData = (ratings, filterStatus, hiddenNames) => (line 1192)
 *   - if (filterStatus === FILTER_OPTIONS.STATUS.ACTIVE) (line 1195)
 *   - if (filterStatus === FILTER_OPTIONS.STATUS.HIDDEN) (line 1198)
 *   - useEffect(() => (line 1236)
 *   - const fetchHistory = async () => (line 1237)
 *   - setIsLoading(true); (line 1238)
 *   - setRatingHistory(history); (line 1245)
 *   - setIsLoading(false); (line 1249)
 *   - fetchHistory(); (line 1253)
 *   - if (!ratingHistory.length) (line 1258)
 *   - switch (timeRange) (line 1265)
 *   - if (!filteredHistory.length) (line 1287)
 *   - if (selectedName) (line 1292)
 *   - if (!acc[item.name]) (line 1311)
 *   - if (isLoading) (line 1350)
 *   - if (!filteredHistory.length) (line 1356)
 *   - if (!ratings.length) (line 1479)
 *   - if (!ratings.length) (line 1535)
 *   - const Profile = ( (line 1605)
 *   - if (hiddenError) (line 1641)
 *   - setHiddenNames(newHiddenNames); (line 1648)
 *   - if (fetchError) (line 1673)
 *   - setCurrentUserRatings(processedRatings); (line 1690)
 *   - if (fetchError) (line 1713)
 *   - setUserLastActivity(lastActivity); (line 1718)
 *   - setAllUsersRatings(ratingsByUser); (line 1719)
 *   - useEffect(() => (line 1728)
 *   - if (ratingsData?.length > 0) (line 1729)
 *   - setCurrentUserRatings(ratingsData); (line 1730)
 *   - useEffect(() => (line 1734)
 *   - if (isAdmin) (line 1735)
 *   - fetchAllUsersRatings(); (line 1736)
 *   - fetchHiddenNames(); (line 1737)
 *   - useEffect(() => (line 1741)
 *   - fetchHiddenNames(); (line 1742)
 *   - if (currentlyViewedUser) (line 1743)
 *   - fetchUserRatings(currentlyViewedUser); (line 1744)
 *   - async (action, user) => (line 1749)
 *   - if (action === "view") (line 1750)
 *   - setCurrentlyViewedUser(user.name); (line 1751)
 *   - async (nameId) => (line 1775)
 *   - if (error) (line 1790)
 *   - if (hiddenNames.size === 0) (line 1804)
 *   - if (error) (line 1820)
 *   - setHiddenNames(new Set()); (line 1823)
 *   - setHiddenRefreshToken((t) => t + 1); (line 1825)
 *   - async (nameId) => (line 1832)
 *   - setDeleteNameStatus( (line 1834)
 *   - if (error) (line 1836)
 *   - setShowDeleteNameConfirm(false); (line 1841)
 *   - setDeleteNameStatus( (line 1843)
 *   - if (filterStatus === FILTER_OPTIONS.STATUS.ALL) (line 1852)
 *   - if (!r || !r.name) (line 1866)
 *   - if (ratingsLoading) (line 1907)
 *   - if (ratingsError) (line 1910)
...
const Profile = ({ userName, onStartNewTournament }) => {
  ...
  // Data fetching
  const {
    storedValue: ratingsData,
    setValue: setRatingsData,
    loading: ratingsLoading,
    error: ratingsError,
    fetchData: refreshRatings,
  ...
  return (
    <div className={styles.profileContainer}>
        ...
        <div className={styles.headerActions}>
          ...
          {isAdmin && (
            <Button
              ...
              title={
                hiddenNames.size === 0 ? "No hidden names" : "Unhide all names"
              }
            >
              Unhide All Names
            </Button>
      ...
      <Modal title="Delete Name" isOpen={showDeleteNameConfirm}>
        <p>
